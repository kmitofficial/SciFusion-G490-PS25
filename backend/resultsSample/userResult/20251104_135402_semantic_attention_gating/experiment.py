...
class SemanticGatingModule(nn.Module):
    ...
    def __init__(self, config: BertConfig):
        ...
        # Project to attention_head_size to keep the module small
        self.query_g = nn.Linear(config.hidden_size, self.attention_head_size)
        self.key_g = nn.Linear(config.hidden_size, self.attention_head_size)
        
        # Learnable scaling factor for the gating scores
    ...
    def forward(self, query_layer: torch.Tensor, key_layer: torch.Tensor) -> torch.Tensor:
        ...
        # Reshape to (batch_size * num_heads, seq_len, head_dim) for linear layers
        query_layer_flat = query_layer.permute(0, 2, 1, 3).reshape(batch_size * seq_len, num_heads, head_dim) # (B*S, N_h, H_d)
        key_layer_flat = key_layer.permute(0, 2, 1, 3).reshape(batch_size * seq_len, num_heads, head_dim)   # (B*S, N_h, H_d)

        # Apply linear layers, projecting from head_dim to head_dim
        # Reshape for linear layer: (B*N_h, S, H_d) -> (B*S, H_d)
        query_g_proj = self.query_g(query_layer.permute(0,2,1,3).reshape(-1, self.hidden_size)).reshape(batch_size, seq_len, num_heads, self.attention_head_size).permute(0,2,1,3)
        key_g_proj = self.key_g(key_layer.permute(0,2,1,3).reshape(-1, self.hidden_size)).reshape(batch_size, seq_len, num_heads, self.attention_head_size).permute(0,2,1,3)
        ...
        # Scale the gating scores with learnable scale_factor and apply sigmoid for gating factors between 0 and 1
