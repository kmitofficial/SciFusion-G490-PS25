...
class SemanticGatingModule(nn.Module):
    ...
    def forward(self, query_layer: torch.Tensor, key_layer: torch.Tensor) -> torch.Tensor:
        ...
        # Compute semantic gating scores directly using dot product
        # (B, N_h, S, H_d) @ (B, N_h, H_d, S) -> (B, N_h, S, S)
        gating_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))
        
        # Scale the gating scores and apply tanh for gating factors between -1 and 1
        gating_scores = torch.tanh(gating_scores * self.scale_factor)
